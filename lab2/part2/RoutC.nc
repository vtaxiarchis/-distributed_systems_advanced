...
implementation
{
  ...
  /* Node to send messages to for routing towards  */
  int16_t router = -1;
  ...
  /* If node is a Cluster Head */
  bool isCluster = FALSE;
  uint16_t clusterElec = 7;
...
#define dbgMessageLineInt(channel,str1,mess,str2,num) dbg(channel,"%s{%d, %s, %d}%s%d\n", str1, mess->from, messageTypeString(mess->type),mess->seq,str2,num);
  ...
  void sendMessage(am_addr_t receiver) {
	...
	/* Emptying the aggregated message count */ 
        msgcount=1;
	break;
      default:
  ...
  void rout() {
	...
	/* Checking if we should route to cluster head or to router */
	  if (m.content == 1 && clusterHead != -1){   
   ...
   */
  void sendAnnounce(uint32_t rounds) {
  ...
  void announceReceive(rout_msg_t *mess) {
     ...
     */
      ...
      if(router == -1 && myd > d) {
	router = mess->from;
	/* In case first announcement is from a  cluster head */
        if(message->clHead == -1){
          if(clusterHead == 0 && myd > d){
      ...
      // After we chose a router we continue with a better routing algorithm
        ...
        // Battery Cost to Current Cluster Head
        int16_t battery_toOldC = batteryRequiredForSend(clusterHead);
        // Distance of the Clusterhead to the sink
        int16_t cluster_distance_to_sink = distance(clusterHead);
        ...
        // If the distance of the router plus the battery is greater than or equal to the distance
        // plus the battery of the sender to the sink, then choose this node to be the router
        if((router_distance_to_sink + battery_toOld) > (sender_distance + battery_toNew)){
          router = mess->from;
        }
        /* Searching for optimal cluster head */
        if(message->clHead == -1 && !isCluster){
          if((cluster_distance_to_sink + battery_toOldC) > (sender_distance + battery_toNew)){
            clusterHead = message->clHead;
          }
	if (message->seq > 1 && clusterHead == -2){
	  isCluster = TRUE;
	} 
        }
      }
  ...
  void contentCollect(rout_msg_t *mess) {
    ...
    if(mess->content > 0) {
      collected = collected + mess->content;
    }
    dbg("Sink", "Sink: Have now collected %d pieces of information\n", collected);
  }
  ...
  event message_t* MessageReceive.receive(message_t* bufPtr, void* payload, uint8_t len) {
	...
	/* If the node is a cluster head dont route the message further 
	but we aggregate and send it with our next message*/
	else if(isCluster){ 
	  msgcount++;
	  dbg("Sink", "Sink: MESSAGE COUNT IS YEAH %d\n", msgcount);
 	}
	else {
	contentReceive(mess);
      }
